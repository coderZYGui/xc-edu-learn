 ...
 *
 * @author zygui
 * @date Created on 2020/6/5 11:57
 */
@Service
public class MediaUploadService {
    ...
    public ResponseResult register(String fileMd5, String fileName, Long fileSize, String mimetype, String fileExt) {
        ...
        //1  检查文件在磁盘上是否存在
        //文件所属目录的路径(如果不存在该路径,就会创建)
        String fileFolderPath = this.getFileFolderPath(fileMd5);
        //文件的路径
        String filePath = this.getFilePath(fileMd5, fileExt);
        File file = new File(filePath); // 根据文件路径来创建该文件的实例
        //判断文件是否存在
        boolean exists = file.exists();

        //2 检查文件信息在mongodb中是否存在
        Optional<MediaFile> optional = mediaFileRepository.findById(fileMd5);
        // 文件在文件目录中存在 且 文件信息也在mongodb数据库中存在
    ...
    public ResponseResult mergechunks(String fileMd5, String fileName, Long fileSize, String mimetype, String fileExt) {
        ...
        //创建一个合并文件
        String filePath = this.getFilePath(fileMd5, fileExt); // 获取文件的路径
