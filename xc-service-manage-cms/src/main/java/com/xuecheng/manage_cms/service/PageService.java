 ...
 *
 * @author zygui
 * @date Created on 2020/5/7 21:31
 */
@Service
public class PageService {
     ...
     * 新增页面
     *
     * @param cmsPage 前台传过来的页面对象,已经在Controller将JSON转为Java对象了
     * @return
     */
/*    public CmsPageResult add(CmsPage cmsPage) {
        // 校验页面名称、站点ID、页面的webpath的唯一性
        // 根据页面名称、站点ID、页面的webpath去cms_page集合,如果查到说明此页面已经存在,如果查询不到就不添加
        CmsPage cmsPage1 = cmsPageRepository.findByPageNameAndSiteIdAndPageWebPath(cmsPage.getPageName(),
                cmsPage.getSiteId(), cmsPage.getPageWebPath());
        if (cmsPage1 == null) {
            // 调用dao新增页面
            cmsPage.setPageId(null); // 防止新建的页面有主键,设置为null,让mongodb数据库自动生成主键
    ...
    public CmsPostPageResult postPageQuick(CmsPage cmsPage) {
        ...
        // 拼接页面URL, 课程详情的访问路径
        // 课程详情的访问路径 = cmsSite.siteDomain+cmsSite.siteWebPath+ cmsPage.pageWebPath + cmsPage.pageName
        // 获取站点id,根据站点id获取站点信息
